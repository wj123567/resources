...
const Supplier = function (supplier) {
    ...
    this.photo_url = supplier.photo_url;
};
...
Supplier.updateById = (id, supplier, result) => {
    const dbConn = Supplier.dbConnect();
    dbConn.query(
        "UPDATE students SET name = ?, city = ?, address = ?, email = ?, phone = ?, state = ?, photo_url = ? WHERE id = ?",
        [supplier.name, supplier.city, supplier.address, supplier.email, supplier.phone, supplier.state, supplier.photo_url, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(err, null);
